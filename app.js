...
const btnTbl = document.querySelector('[data-js="tbl-btn"]')
let tblUsuario = document.querySelector('[data-js="tabela"]')
let arr = []

btnMinimizar.addEventListener("click",()=>{
  formUsuarios.classList.add("form-fechado")
  tblUsuario.classList.add("tbl-usuario")
  tblUsuario.classList.add("visivel-tabela-on")
  
})

btnTbl.addEventListener('click',()=>{
  formUsuarios.classList.remove("form-fechado")
  formUsuarios.classList.add("visivel-cadastro-on")
  tblUsuario.classList.add("tbl-usuario")
  tblUsuario.classList.remove("visivel-tabela-on")
})
...
const pagIntefaceInsercao = (arrObj) =>{
  ...
  for(let i=0; i < arrObj.length; i++){
    ...
    const celulaTabelaBTN = document.createElement('td')
      ...
      linhaTabela.append(celulaTabelaName,celulaTabelaBTN)
...
const dasalinhandoObjDoc = (arrObjDoc)=>{
  const propString = 'stringValue';
  const propNumber = 'integerValue';
  const novoArrObj = arrObjDoc.map(({usuario, endereco,id})=>{
     const {bairro,cep,complemento,localidade,logradouro,numero,tipo_logradouro,uf} = endereco.mapValue.fields
     const {nome,sobrenome,idade,sexo_select,horaCriacao} = usuario.mapValue.fields
     
      return{
        nome: nome[propString],
        sobrenome:sobrenome[propString],
        idade:idade[propNumber],
        sexo_select:sexo_select[propString],
        horaCriacao: formatandoData(horaCriacao['timestampValue']),
        id:id,
        endereco:{
          bairro:bairro[propString],
          cep:parseInt(cep[propString]),
          complemento:complemento[propString],
          cidade:localidade[propString],
          logradouro:logradouro[propString],
          numero: parseInt(numero[propString]),
          tipoLogradouro: tipo_logradouro[propString],
          estado: uf[propString]
        }
      }
...
onSnapshot(collectionsBd,(dados)=>{
  ...
  if(!dados.metadata.hasPendingWrites){
    ...
    const arrObjDestructorDocs = docs.map((docItem)=>{
      const objUsuario = docItem._document.data.value.mapValue.fields
      const id = docItem.id
      const { usuario, endereco} = objUsuario
     return {usuario,endereco,id}
    })
    dasalinhandoObjDoc(arrObjDestructorDocs)
  }
